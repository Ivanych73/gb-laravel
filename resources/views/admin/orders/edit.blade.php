...
@section('content')
    ...
    <div class="container">
        ...
        <form method="post" action="{{ route('admin.orders.update', ['order' => $order]) }}">
            ...
            <div class="mb-3">
                <span class="lead">Email:&nbsp{{ $order->email }}</span>
            </div>
            <div class="mb-3">
                <span class="lead">Телефон:&nbsp{{ $order->phone }}</span>
            </div>
            <div class="mb-3">
            ...
            <div class="input-group mb-3">
                ...
                <select class="custom-select" id="statusSelect" name="status">
                    ...
                    <option value="processed" @if ($order->status == 'processed') selected @endif>В работе</option>
                    <option value="success" @if ($order->status == 'success') selected @endif>Успешно завершен</option>
                    <option value="fail" @if ($order->status == 'fail') selected @endif>Завершен неудачно</option>
                </select>
            </div>
